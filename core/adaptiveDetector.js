...
function meanAndVariance(values) {
    ...
    let sq = 0;
    for (let i = 0; i < values.length; i++) { const d = values[i] - mean; sq += d * d; }
...
function normalizedCrossCorrelation(a, b) {
    ...
    let num = 0;
    for (let i = 0; i < a.length; i++) num += (a[i] - statsA.mean) * (b[i] - statsB.mean);
    return num / den;
}
...
function getRegion(data, width, x, y, size) {
    ...
    for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) out[dstBase + col] = data[srcBase + col];
    }
    return out;
}
...
function toRegionGrayscale(imageData, region) {
    ...
    if (region.x < 0 || region.y < 0 || region.x + size > width || region.y + size > height) return new Float32Array(0);
    const out = new Float32Array(size * size);
    for (let row = 0; row < size; row++) {
...
function stdDevRegion(data, width, x, y, size) {
    ...
    for (let row = 0; row < size; row++) {
        ...
        for (let col = 0; col < size; col++) { const v = data[base + col]; sum += v; sq += v * v; n++; }
    }
    if (n === 0) return 0;
    const mean = sum / n;
...
function buildTemplateGradient(alphaMap, size) { return sobelMagnitude(alphaMap, size, size); }
...
function scoreCandidate({ gray, grad, width, height }, alphaMap, templateGrad, candidate) {
    ...
    if (y > 8) {
        ...
        if (refH > 8) {
            ...
            if (refStd > EPSILON) varianceScore = clamp(1 - wmStd / refStd, 0, 1);
        }
    }

    const confidence = Math.max(0, spatial) * 0.5 + Math.max(0, gradient) * 0.3 + varianceScore * 0.2;
    return { confidence: clamp(confidence, 0, 1), spatialScore: spatial, gradientScore: gradient, varianceScore };
}
...
export function warpAlphaMap(alphaMap, size, { dx = 0, dy = 0, scale = 1 } = {}) {
    ...
    if (!Number.isFinite(dx) || !Number.isFinite(dy) || !Number.isFinite(scale) || scale <= 0) return new Float32Array(0);
    if (dx === 0 && dy === 0 && scale === 1) return new Float32Array(alphaMap);

    const sample = (x, y) => {
        const x0 = Math.floor(x), y0 = Math.floor(y);
        const fx = x - x0, fy = y - y0;
        const ix0 = clamp(x0, 0, size - 1), iy0 = clamp(y0, 0, size - 1);
        const ix1 = clamp(x0 + 1, 0, size - 1), iy1 = clamp(y0 + 1, 0, size - 1);
        const p00 = alphaMap[iy0 * size + ix0], p10 = alphaMap[iy0 * size + ix1];
        const p01 = alphaMap[iy1 * size + ix0], p11 = alphaMap[iy1 * size + ix1];
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            out[y * size + x] = sample((x - c) / scale + c + dx, (y - c) / scale + c + dy);
        }
    }
    return out;
}
...
export function interpolateAlphaMap(sourceAlpha, sourceSize, targetSize) {
    ...
    for (let y = 0; y < targetSize; y++) {
        const sy = y * scale, y0 = Math.floor(sy), y1 = Math.min(sourceSize - 1, y0 + 1), fy = sy - y0;
        for (let x = 0; x < targetSize; x++) {
            const sx = x * scale, x0 = Math.floor(sx), x1 = Math.min(sourceSize - 1, x0 + 1), fx = sx - x0;
            const p00 = sourceAlpha[y0 * sourceSize + x0], p10 = sourceAlpha[y0 * sourceSize + x1];
            const p01 = sourceAlpha[y1 * sourceSize + x0], p11 = sourceAlpha[y1 * sourceSize + x1];
            out[y * targetSize + x] = (p00 + (p10 - p00) * fx) + ((p01 + (p11 - p01) * fx) - (p00 + (p10 - p00) * fx)) * fy;
        }
    }
    return out;
}
...
export function computeRegionGradientCorrelation({ imageData, alphaMap, region }) {
    ...
    return normalizedCrossCorrelation(sobelMagnitude(patch, size, size), sobelMagnitude(alphaMap, size, size));
}

export function shouldAttemptAdaptiveFallback({ processedImageData, alphaMap, position, residualThreshold = 0.22, originalImageData = null, originalSpatialMismatchThreshold = 0 }) {
    const residualScore = computeRegionSpatialCorrelation({
        imageData: processedImageData, alphaMap,
        region: { x: position.x, y: position.y, size: position.width ?? position.size }
    });
    if (residualScore >= residualThreshold) return true;
    if (originalImageData) {
        const originalScore = computeRegionSpatialCorrelation({
            imageData: originalImageData, alphaMap,
            region: { x: position.x, y: position.y, size: position.width ?? position.size }
        });
        if (originalScore <= originalSpatialMismatchThreshold) return true;
    }
    return false;
}
...
export function detectAdaptiveWatermarkRegion({ imageData, alpha96, defaultConfig, threshold = DEFAULT_THRESHOLD }) {
    ...
    let best = defaultScore
        ? { ...defaultCandidate, ...defaultScore }
        : { ...defaultCandidate, confidence: 0, spatialScore: 0, gradientScore: 0, varianceScore: 0 };
        ...
        for (let size = scaleLo; size <= scaleHi; size += 2) {
            ...
            for (let x = coarse.x - 8; x <= coarse.x + 8; x += 2) {
                ...
                for (let y = coarse.y - 8; y <= coarse.y + 8; y += 2) {
                    ...
                    const score = scoreCandidate(context, tpl.alpha, tpl.grad, { x, y, size });
                    if (!score) continue;
                    if (score.confidence > best.confidence) best = { x, y, size, ...score };
                }
            }
        }
    }
